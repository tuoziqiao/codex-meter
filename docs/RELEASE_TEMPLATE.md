...
CodexMeter is a lightweight floating desktop widget for checking Codex quota from the local Codex Desktop login state.

## Downloads

- Windows unsigned: `codex-meter-windows-unsigned.zip`
- macOS Universal unsigned: `codex-meter-macos-universal-unsigned.zip`
...
CodexMeter does not store Codex tokens, account IDs, prompts, chats, raw quota responses, or local auth paths. It stores only widget preferences. See `PRIVACY.md`.
